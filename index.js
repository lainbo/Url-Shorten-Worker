const config = {
  ...
  async function handleRequest(request) {
    ...
    if (request.method === "POST") {
      ...
      if (req_cmd == "add") {
        ...
        if(!await checkURL(req_url)){
          return new Response(`{"status":500,"key": "", "error":"Error: 原始链接不合法，请填写完整正确的链接."}`, {
            headers: response_header,
          })
        }
  
        if (req_password != password_value) {
          return new Response(`{"status":500,"key": "", "error":"Error: 密码错误."}`, {
            headers: response_header,
          })
        }
        ...
        if (config.custom_link && (req_keyPhrase != "")){
          ...
          if (is_exist != null) {
            return new Response(`{"status":500,"key": "", "error":"Error: 自定义后缀已经存在."}`, {
              headers: response_header,
            })
          }else{
        ...
        if (typeof(stat) == "undefined"){
          ...
          return new Response(`{"status":500, "key": "", "error":"Error:数据库操作上限, 请次日再试."}`, {
            headers: response_header,
          })
        }
      } else if (req_cmd == "del") {
    ...
    if (path==password_value){  
      let index= await fetch(`https://raw.githubusercontent.com/lainbo/Url-Shorten-Worker/crazypeace-gh-pages/theme/${config.theme}/index.html`)
